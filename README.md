...
[MIT License](LICENSE) © 2026 Battlemagnet1 —— 可以自由使用、修改、分发（含商用），只需保留版权声明与许可声明。

**注意**：许可证只覆盖**本仓库的源代码与文档**。示例照片来自 Wikimedia Commons（各自适用其原始许可）；
应用运行时调用的 AI 服务（通义千问 / 豆包 / OpenAI 等）由各自的服务条款约束，与本仓库无关。
